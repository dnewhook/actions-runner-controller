...
RUN dnf -y update; yum -y reinstall shadow-utils; \
    dnf -y install procps-ng crun podman podman-docker buildah containernetworking-plugins fuse-overlayfs /etc/containers/storage.conf --exclude container-selinux \
...
ADD podman-containers-cni.conf /home/runner/.config/containers/containers.conf
